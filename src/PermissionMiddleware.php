...
class PermissionMiddleware
{
    ...
    public function getRedirectResponse(Request $request, Response $response, string $url = "<#= loginPage #>")
    {
        $isModal = $request->getQueryParam("modal") == "1";
        if ($isModal) {
            if ($url == "<#= loginPage #>" && Config("USE_MODAL_LOGIN")) { // Redirect to login
