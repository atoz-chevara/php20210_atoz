<## Common config #>
<#
    // Encoding from project charset (iconv assumed)
    let encoding = "";
    if (!IsEmpty(PROJ.Encoding))
        encoding = PROJ.Encoding;
    if (encoding == "")
        encoding = CharsetToIconvEncoding(PROJ.CharSet);
#>
<?php
...
// Product version
define(PROJECT_NAMESPACE . "PRODUCT_VERSION", "<#= Quote(PROJ.Version) #>");

// Project
define(PROJECT_NAMESPACE . "PROJECT_NAME", "<#= PROJ.ProjVar #>"); // Project name
define(PROJECT_NAMESPACE . "PROJECT_ID", "<#= PROJ.ProjID #>"); // Project ID
 ...
 * details.
 */
define(PROJECT_NAMESPACE . "PROJECT_ENCODING", "<#= encoding #>"); // Character encoding
define(PROJECT_NAMESPACE . "IS_DOUBLE_BYTE", in_array(PROJECT_ENCODING, ["GBK", "BIG5", "SHIFT_JIS"])); // Double-byte character encoding
define(PROJECT_NAMESPACE . "FILE_SYSTEM_ENCODING", "<#= PROJ.FileSystemEncoding #>"); // File system encoding

// Database
define(PROJECT_NAMESPACE . "IS_MSACCESS", <#= Code.bool(IsMsAccess()) #>); // Access
define(PROJECT_NAMESPACE . "IS_MSSQL", <#= Code.bool(IsMsSQL()) #>); // SQL Server
define(PROJECT_NAMESPACE . "IS_MYSQL", <#= Code.bool(IsMySQL()) #>); // MySQL
define(PROJECT_NAMESPACE . "IS_POSTGRESQL", <#= Code.bool(IsPostgreSQL()) #>); // PostgreSQL
define(PROJECT_NAMESPACE . "IS_ORACLE", <#= Code.bool(IsOracle()) #>); // Oracle
define(PROJECT_NAMESPACE . "IS_SQLITE", <#= Code.bool(IsSQLite()) #>); // SQLite
define(PROJECT_NAMESPACE . "DB_QUOTE_START", "<#= Quote(DB.DBQuoteS) #>");
define(PROJECT_NAMESPACE . "DB_QUOTE_END", "<#= Quote(DB.DBQuoteE) #>");
