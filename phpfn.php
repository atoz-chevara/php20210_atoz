...
<#
    let authenticateMode = PROJ.AuthenticateMode,
        rootRelativePath = GetRootRelativePath();
#>
<?php
...
function ConnectDb($info)
{
    ...
    <# if (ServerScriptExist("Global", "Database_Connecting")) { #>
    ...
    $dbtype = @$info["type"];
    if ($dbtype == "MYSQL") {
        $info["driver"] = $info["driver"] ?? "<#= PROJ.MySqlDriver || "mysqli" #>";
        ...
        $info["driver"] = $info["driver"] ?? "<#= PROJ.MsSqlDriver || "sqlsrv" #>";
        ...
        $info["driver"] = "oci8";
        <# if (PROJ.OracleCharset != "") { #>
        if (!array_key_exists("charset", $info)) {
            $info["charset"] = "<#= PROJ.OracleCharset #>";
        }
        <# } #>
    }
    ...
    if ($dbtype == "MYSQL" || $dbtype == "POSTGRESQL" || $dbtype == "ORACLE") {
        ...
        if ($dbtype == "ORACLE") {
            ...
            $oraVars["NLS_COMP"] = "<#= PROJ.OracleCompare #>";
            <# } #>
            <# if (PROJ.OracleSort != "") { #>
            $oraVars["NLS_SORT"] = "<#= PROJ.OracleSort #>";
            <# } #>
    ...
    <# if (ServerScriptExist("Global", "Database_Connected")) { #>
...
function SendEmail($fromEmail, $toEmail, $ccEmail, $bccEmail, $subject, $mailContent, $format, $charset, $smtpSecure = "", $attachments = [], $images = [], $members = null)
{
    ...
    if (SameText($format, "html")) {
        ...
        $mail->AltBody = HtmlToText($mailContent);
        <# } #>
    } else {
        $mail->isHTML(false);
...
function RootRelativePath()
{
    ...
    <# if (rootRelativePath) { #>
    return $RELATIVE_PATH ? PathCombine($RELATIVE_PATH, "<#= rootRelativePath #>", false) : "";
    <# } else { #>
...
function SetupLoginStatus()
{
    ...
    $logoutPage = "<#= BaseName(logoutPage, ".php") #>";
    ...
    $LoginStatus["canLogout"] = $logoutPage && IsLoggedIn() && CurrentUserName() != CurrentWindowsUser();
    <# } else { #>
    ...
    $LoginStatus["changePasswordText"] = $Language->phrase("ChangePassword");
        <# if (PROJ.AuthenticateMode == "Windows") { #>
    $LoginStatus["canChangePassword"] = $changePasswordUrl && IsLoggedIn() && !IsSysAdmin() && !IsAuthenticated();
        <# } else { #>
